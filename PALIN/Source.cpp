...
public:
	...
	void turnIntoPalindrome()
	{
		...
		{
			// data | data
			int middleMoreSignificant = numElems / 2; //no real middle element exists here
			bool findingMode = true;
			for (int i = 0; i < middleMoreSignificant; i++) // middleMoreSignificant = half elemnets
			{
				...
				if (findingMode) {
					...
					{
						if (right > left)
							incrementMiddle(middleMoreSignificant);
...
private:
	...
	void incrementMiddle(int middle)
	{
		...
		while (true)
		{
			...
			{
				//simple increment would doubly increment our value in case middleMore = middleLess
	...
	static int moreSignificantBit(int pos, int shift)
	{
		return pos+shift;
	}
...
void test(int count)
{
	...
	for(int i=0;i<count;i++)
	{
		...
		while (!naive.isPalindrome())
			++naive;

		if (fast != naive)
			cout << endl << "For " << randomInt << " expected " << naive << " but got " << fast << endl;

		if (i % (count / 100) == 0)
			cout << i / (double)count * 100 << "\r";
	}
	cout << endl;
	exit(0);
}
